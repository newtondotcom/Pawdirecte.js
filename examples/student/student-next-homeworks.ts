...
void (async function main() {
  ...
  const homeworkId = Number.parseInt(
    prompt("Enter homework id to toggle done state:") ?? "0"
  );
  ...
  const freshNextAssignements = (
    await studentComingHomeworks(session, account)
  ).find((v) => {
    return v.homeworks.find((h) => h.id === homeworkId);
  });
  const homework = freshNextAssignements?.homeworks.find(
    (h) => h.id === homeworkId
  );
  if (homework) {
    console.log(
