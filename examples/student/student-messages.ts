...
void (async function main() {
  ...
  for (const message of messages.chats) {
    console.log(
      `${message.id}: Mail from ${message.sender} at ${message.date.toLocaleDateString("fr-CA")}: ${message.subject} Attachements: ${message.files.length > 0}`
    );
  }
  ...
  for (const index in message.files) {
    console.log(
      `${index}: ${message.files[index].name}, ${message.files[index].type}, ${message.files[index].id}`
    );
  }
  const fileId = Number.parseInt(
    prompt("Which file do you wish to dl:") ?? "0"
  );
