import { FileKind, getFile, studentDocuments } from "~/api";
...
void (async function main() {
  ...
  for (const document of documents) {
    console.log(
      `[${documents.indexOf(document)}] ${document.name}, ${document.date} (${document.kind})`
    );
  }
  const documentId = Number.parseInt(prompt("Which document to dl:") ?? "0");
